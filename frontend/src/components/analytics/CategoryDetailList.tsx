...
export default function CategoryDetailList({
  ...
  if (cats.length === 0) {
    return (
      <div className="bg-bg-white rounded-xl p-10 border border-border-default text-center text-text-muted text-[14px]">
    ...
    <div className="flex-1">
      {cats.map((cat, ci) => {
        ...
        return (
          <div
            key={ci}
            className="bg-bg-white rounded-xl px-[22px] py-[18px] border border-border-default mb-3"
          >
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <span
                className={["text-[13px] font-bold", amountColorClass].join(
                  " ",
                )}
              >
                {formatRp(isExpense ? -cat.amount : cat.amount)}
              </span>
              <span className="text-[11px] px-2 py-0.5 rounded-[10px] bg-bg-primary text-text-secondary">
                {pct}% of total
              </span>
            </div>
            ...
            {cat.subs.map((s, si) => (
              <div key={si} className="flex items-center gap-2 py-1.5">
                ...
                <span className="text-[12px] font-medium text-text-muted min-w-[70px] text-right">
