...
function CollapsePanel({ open, children, style }: { open: boolean; children: React.ReactNode; style?: React.CSSProperties }) {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#F5F5F2', overflow: 'hidden' }}>
      ...
      <div className="tx-summary">
        {/* Desktop layout */}
        <div className="tx-summary-inner tx-summary-desktop" style={{ display: 'flex' } as React.CSSProperties}>
          {[
        ...
        >
          <div className="tx-summary-net-row">
            <div>
              <div style={{ fontSize: 10, fontWeight: 600, color: '#bbb', letterSpacing: '0.08em', marginBottom: 4 }}>NET</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: net >= 0 ? '#111' : '#E05C5C', letterSpacing: '-0.02em' }}>
                {formatCurrency(net)}
              </div>
            </div>
            <button
            ...
            >
              <svg
                ...
                style={{ transform: summaryExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }}
              >
          ...
          <CollapsePanel open={summaryExpanded}>
            <div className="tx-summary-sub">
              {[
                ...
                { label: 'OUTFLOW', value: outflow, color: '#E05C5C', sign: '-' },
              ].map((item, i) => (
                <div key={i} className="tx-summary-item" style={{ flex: 1 }}>
                  <div style={{ flex: 1, textAlign: i === 0 ? 'left' : 'right' }}>
                    <div style={{ fontSize: 10, fontWeight: 600, color: '#bbb', letterSpacing: '0.08em', marginBottom: 4 }}>{item.label}</div>
                    <div style={{ fontSize: 16, fontWeight: 700, color: item.color, letterSpacing: '-0.02em' }}>
        ...
        {!loading && groups.map(([date, txs]) => {
          ...
          return (
            <div key={date} style={{ marginBottom: 12, borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', background: '#EEEEE8' }}>
              ...
              {txs.map((tx, i) => {
                ...
                return (
                  <div key={tx.id}
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 16px', background: i % 2 === 0 ? 'white' : '#FAFAF7', borderBottom: isLast ? 'none' : '1px solid #F2F2EE', cursor: 'pointer', transition: 'background 0.1s' }}
                  ...
                  >
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#F0F0EE', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: '#888', flexShrink: 0 }}>
                    ...
                    {tx.subCategory && (
                      <div style={{ padding: '3px 8px', borderRadius: 20, background: '#F2F2EE', fontSize: 11, color: '#888', flexShrink: 0, maxWidth: 100, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{tx.subCategory.name}</div>
                    )}
                    <div className="tx-row-amount" style={{ color: signed < 0 ? '#E05C5C' : '#2A9D5C' }}>
